...
3. AI が acceptance/ へ翻訳する（接続先は ACCEPTANCE_BASE_URL から取る）
...
`spec/*` から `backend/` `frontend/` も書けない。CI が「参照モックに対して緑」を要求する。
...
|---|---|
| 仕様表が未承認 | Phase A に戻る |
| **参照モックで緑にならない** | **翻訳のバグ。** 実装のせいにしない |
| **backend で緑になってしまう** | 実装済みか、テストが何も検証していない |
