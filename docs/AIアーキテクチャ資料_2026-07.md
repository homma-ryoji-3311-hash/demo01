...
date: 2026-07-12
source: M-team開発計画書_v2.md（v2.5 確定版）
status: 説明資料（正本ではない。食い違ったら計画書・ADR・playbook が正）
tags: [m-team, aiアーキ, アーキテクチャ資料, harness]
---

# M-team AIアーキテクチャ資料

> **この資料の位置づけ**：計画書 v2.5 からアーキテクチャ部分を抜き出した説明資料。
...
- 既製は mattpocock/skills v1.1（`/tdd`・`/diagnosing-bugs` ほか）。**丸ごと導入せず部品抜粋**が基本方針（GSD アーカイブ事件＝サプライリスクの実例）
...
```mermaid
flowchart TD
    subgraph UP["上流（毎スライス）"]
        A(["基本設計＝answer key 書き起こし"]) --> C["issue 分解（縦切り）＋ /board 採番"]
        C --> S1["/spec PhaseA：仕様表（grill）"]
        S1 --> AP{"PM が approved: true"}
        AP --> S2["/spec PhaseB：翻訳＋golden"]
        S2 --> BR["/brief：指示書＋起票"]
    end

    subgraph DOWN["下流（AFK）＝ /slice"]
    ...
    subgraph GATE["関所（HITL）＝ /integrate"]
        J["PhaseA：統合役が再検証 → 停止"]
        K{"層境ゲート：PM GO/NO-GO"}
        L["PhaseB：gh pr merge（server-side）"]
    ...
    class A,C,S1,S2,BR,AP,Done up
    class E,F,H,V down
    class J,K,L,E2E,Q gate
    class M,RG fw
```
...
- 計画書正本：Obsidian Vault `M-team/AIアーキ/正本/M-team開発計画書_v2.md`（v2.5）
- 憲法：`CLAUDE.md`／手順：`docs/playbook.md`／決定：`docs/adr/`（0001, 0004〜0014）
