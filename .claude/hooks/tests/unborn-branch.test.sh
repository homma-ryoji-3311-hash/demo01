...
# --- 4) protect-paths: detached HEAD で backend/ 書込が fail-closed でブロック ---
...
RC="$(run_hook protect-paths.sh "$REPO_D" '{"tool_input":{"file_path":"'"$REPO_D"'/backend/x.ts"}}')"
[[ "$RC" == "2" ]] \
  && pass "detached HEAD での backend/ 書込は block (exit 2)" \
  || fail "detached HEAD での backend/ 書込が素通り (exit $RC) <- fail-open"
