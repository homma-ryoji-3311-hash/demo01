...
RC="$(run_hook protect-paths.sh "$REPO_BOOT" '{"tool_input":{"file_path":"'"$REPO_BOOT"'/backend/x.ts"}}')"
[[ "$RC" == "2" ]] \
  && pass "feature/slice-bootstrap での backend/ 書込は block (exit 2)" \
  || fail "feature/slice-bootstrap での backend/ 書込が素通り (exit $RC) <- fail-open"

# --- 7) protect-paths: 正規の feature ブランチは backend/ 可・acceptance/ 不可（従来動作の維持） ---
RC="$(run_hook protect-paths.sh "$REPO_OK" '{"tool_input":{"file_path":"'"$REPO_OK"'/backend/x.ts"}}')"
[[ "$RC" == "0" ]] \
  && pass "feature/slice-01 での backend/ 書込は allow (exit 0)" \
  || fail "feature/slice-01 での backend/ 書込が誤ブロック (exit $RC)"
