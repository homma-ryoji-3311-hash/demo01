...
- **既存の番号を振り直さない（never renumber）。** `NN` はブランチ名・`acceptance/` パス・仕様表・golden 名・issue に焼き込まれている。振り直しは全参照を孤児にする。
...
| `approved: true`・`spec/slice-NN` ブランチあり・acceptance 未マージ | 工程4（翻訳中 or PR 中） |
| acceptance が main にあり・指示書 `docs/slices/slice-NN.md` が無い | 工程5 前 |
