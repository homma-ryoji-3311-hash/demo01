...
- 実装（`backend/` `frontend/`）と `acceptance/` に触らない。
...
2. `acceptance/` に当該スライスの失敗するテストがある（工程4 済み）。
...
| 2. 受け入れテスト | PM＋AIアーキ | `acceptance/` の実パス・golden 名・仕様表パスを列挙 |
| 3. 触ってよいファイル範囲 | AIアーキ | `router → service → repository` の構造規約（ADR-0011）に沿った具体パス |
| 4. 貼り付け用の枠 | **リーダー** | 共通部は雛形のまま。**固有注意1〜2行は空欄で残す** |
| 5. 完了の定義 | 共通 | 変えない（3つの機械判定） |
| 6. 禁止事項 | **リーダー** | 共通部＋「着手してはいけない隣接スライス」の候補を提案 |

**ここで停止する。** リーダーに「4 の固有注意と 6 を確定してください」と提示する。
確定後、指示書は `acceptance/` と同じ `spec/slice-NN` ブランチでまとめて PR → main へマージされる（工程4 の PR に同乗、または追補 PR）。
...
受け入れテスト: acceptance/<path>.spec.ts
```
